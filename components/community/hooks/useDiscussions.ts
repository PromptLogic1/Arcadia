...
export const useDiscussions = (): UseDiscussionsReturn => {
  ...
  const transformDatabaseComment = useCallback((dbComment: DatabaseComment & {
    ...
    id: dbComment.id,
    author: dbComment.author.username,
    avatar: dbComment.author.avatar_url || '/default-avatar.png',
  ...
  const transformDatabaseDiscussion = useCallback((dbDiscussion: DiscussionWithRelations): Discussion => ({
    id: dbDiscussion.id,
    author: dbDiscussion.author.username,
    avatar: dbDiscussion.author.avatar_url || '/default-avatar.png',
    title: dbDiscussion.title,
    game: dbDiscussion.game,
    challengeType: dbDiscussion.challenge_type,
    comments: dbDiscussion.comments.count || 0,
    upvotes: dbDiscussion.upvotes,
    content: dbDiscussion.content,
    date: dbDiscussion.created_at,
    ...
    try {
      ...
      const { data, error: supabaseError } = await supabase
        ...
        .insert({
          ...
          challenge_type: discussion.challengeType,
          author_id: userData.user.id,
          tags: Array.from(discussion.tags),
          upvotes: 0
        })
        .select(`
          *,
