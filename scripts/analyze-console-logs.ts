...
interface ConsoleLogInstance {
  ...
  content: string;
  context: string;
  category: 'test-scenario' | 'debug' | 'error-simulation' | 'performance' | 'unknown';
}

const consoleLogs: ConsoleLogInstance[] = [];

function categorizeConsoleLog(content: string, context: string): ConsoleLogInstance['category'] {
  const lowerContent = content.toLowerCase();
  const lowerContext = context.toLowerCase();
  
  // Test scenario logs (simulating real app behavior)
  if (lowerContent.includes('circuit breaker') ||
...
const byCategory = consoleLogs.reduce((acc, log) => {
...
const categories: ConsoleLogInstance['category'][] = ['test-scenario', 'error-simulation', 'performance', 'debug', 'unknown'];

categories.forEach(category => {
  const examples = consoleLogs.filter(log => log.category === category).slice(0, 3);
...
console.log('\nRecommendations:');
console.log('================');
console.log('1. test-scenario logs: Keep these as they simulate real application behavior');
console.log('2. error-simulation logs: Keep these as they are part of error handling tests');
console.log('3. performance logs: Consider replacing with proper performance assertions');
