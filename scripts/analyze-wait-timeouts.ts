...
function analyzeTimeout(
  ...
  // Animation waits
  if (
    contextLower.includes('animation') ||
    ...
    return {
      ...
      canReplace: true,
      suggestion:
        'Replace with: await page.waitForFunction(() => !document.querySelector(".animating"))',
    };
  }

  // Debounce waits
  if (
    contextLower.includes('debounce') ||
  ...
  if (
    ...
    return {
      ...
      canReplace: true,
      suggestion:
        'Replace with: await expect(element).toBeVisible() or waitForFunction',
    };
  }
  ...
  return {
    ...
    suggestion:
      duration < 1000
        ? 'Consider replacing with proper wait condition'
        : undefined,
  };
}
...
const byPurpose = timeouts.reduce(
  (acc, timeout) => {
...
console.log(
  `Potentially replaceable: ${replaceable.length} (${Math.round((replaceable.length / timeouts.length) * 100)}%)\n`
);
...
console.log('\n\nRecommendations:');
console.log(
  '1. Network simulation timeouts: Keep these as they test timing-sensitive behavior'
);
console.log('2. Retry waits: Keep these as they test retry intervals');
console.log(
  '3. Animation waits: Replace with waitForFunction checking animation state'
);
