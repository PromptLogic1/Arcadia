...
const processors: FileProcessor[] = [
  ...
  // Fix any in object properties
  {
    ...
    replacement: (_match, prop) => {
      ...
      if (prop.includes('handler') || prop.includes('callback'))
        return `${prop}: (...args: unknown[]) => unknown;`;
...
function processFile(filePath: string) {
  ...
  const needsTestWindowImport =
    content.includes('window as any') ||
    content.includes('window as TestWindow');

  if (
    needsTestWindowImport &&
    ...
    if (importMatch && importMatch.length > 0) {
      ...
      if (lastImport) {
        ...
        content =
          content.slice(0, index + lastImport.length) +
          '\n' +
          importLine +
          content.slice(index + lastImport.length);
        modified = true;
      }
    }
  }
  ...
  for (const processor of processors) {
    ...
    const newContent =
      typeof processor.replacement === 'string'
