...
const SUPABASE_ANON_KEY =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNub3RpdXBkcWJkeHh4anJjcXZiIiwicm9sZUFub24iLCJpYXQiOjE3NDgyOTA4ODQsImV4cCI6MjA2Mzg2Njg4NH0.Epy3TVOBekPA-lKxm42-res4SOu-qsoQ833T5eyWAwk';
  ...
  try {
    ...
    if (allRecords.length > 0) {
      ...
      Object.entries(firstRecord).forEach(([key, value]) => {
        ...
        console.log(
          `  • ${key}: ${type}${isNull ? ' (nullable)' : ''} - ${sample}`
        );
      });
      ...
      for (const [key, value] of Object.entries(firstRecord)) {
        if (typeof value === 'string' && value.length < 50) {
          const uniqueValues = [
            ...new Set(allRecords.map(record => record[key])),
          ];
  ...
  for (const [tableName, columns] of Object.entries(enumColumns)) {
    try {
      ...
      if (data) {
        ...
        columns.forEach(col => {
          const uniqueValues = [
            ...new Set(data.map(record => record[col]).filter(v => v !== null)),
          ];
