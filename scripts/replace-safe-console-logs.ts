...
const safeReplacements: Replacement[] = [
  ...
  {
    pattern:
      /console\.log\(`([^`]*time|Time|duration|Duration): \${([^}]+)}(ms)?\`\);?/g,
...
function processFile(filePath: string) {
  ...
  if (modified) {
    ...
    console.log(
      `Updated ${filePath} - Replaced ${replacementCount} console.log statements`
    );
  }
}
...
console.log(
  'Keeping: test scenarios, error simulations, and other intentional logs\n'
);
...
console.log(
  'Note: Many console.logs were kept as they simulate real application behavior'
);
