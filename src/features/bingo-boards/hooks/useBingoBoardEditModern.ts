...
import { useBoardEditState, useBoardEditActions, type BoardEditFormData } from '@/lib/stores/board-edit-store';
import { useAuth } from '@/lib/stores/auth-store';
import { logger } from '@/lib/logger';
import { notifications } from '@/lib/notifications';

export interface UseBingoBoardEditModernReturn {
  ...
  updateCard: (cardId: string, updates: Partial<BingoCard>) => Promise<BingoCard | null>;
  
  // Validation
  validateField: (field: string, value: string | string[] | boolean) => string | null;
 ...
 */
export function useBingoBoardEditModern(boardId: string): UseBingoBoardEditModernReturn {
  ...
  const handlePositionSelect = useCallback((index: number) => {
    if (uiState.selectedCard) {
      ...
      const cardElement = document.querySelector(`[data-card-id="${uiState.selectedCard.id}"]`);
      if (cardElement) {
  ...
  const handleSave = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const cardsToSave = [
        ...uiState.localPrivateCards.filter(card => card.id.startsWith('temp-')),
  ...
  }, [board, uiState.formData, uiState.localGridCards, uiState.localPrivateCards, authUser, saveCards, updateBoard, uiActions, boardId]);
  ...
  // Create card wrapper
  const createCard = useCallback(async (cardData: Partial<BingoCard>): Promise<BingoCard | null> => {
  ...
  // Update card wrapper
  const updateCard = useCallback(async (cardId: string, updates: Partial<BingoCard>): Promise<BingoCard | null> => {
    try {
