...
export const useBingoGame = (
  ...
  if (
    initialSize < CONSTANTS.MIN_BOARD_SIZE ||
    initialSize > CONSTANTS.MAX_BOARD_SIZE
  ) {
    throw new Error(
      `Board size must be between ${CONSTANTS.MIN_BOARD_SIZE} and ${CONSTANTS.MAX_BOARD_SIZE}`
    );
  }
  ...
  const handleError = useCallback((error: Error) => {
    if (process.env.NODE_ENV !== 'test') {
      logger.error('Bingo game error', error, {
        metadata: { hook: 'useBingoGame' },
      });
    }
    setGameError({
  ...
  const { updateStats, trackMove, recordWinner, measurePerformance } =
    useGameAnalytics();
    ...
    players.forEach(player => {
      ...
      boardData.rows.forEach(row => {
        if (row?.every(cell => cell?.colors?.includes(player.color || ''))) {
          completedLines[player.user_id] =
            (completedLines[player.user_id] || 0) + 1;
        }
      });
  ...
  // Event Emitters
  const emitBeforeMove = useCallback(
    (event: BeforeMoveEvent) => {
      try {
        ...
        const isValid =
          event.isValid && players[currentPlayer]?.user_id === event.playerId;
  ...
  const emitAfterMove = useCallback(
  ...
  const checkLineWin = useCallback(
    (cells: GameBoardCell[]): boolean => {
      if (!cells || cells.length !== boardSize) return false;

      // Get the first marked cell's completed_by player
      const firstMarkedCell = cells.find(
        cell =>
          cell.is_marked && cell.completed_by && cell.completed_by.length > 0
      );
  ...
  const checkWinningCondition = useCallback(
    (currentPlayers: GamePlayer[], timeExpired?: boolean): boolean => {
      try {
        measurePerformance();
        ...
        if (winConditions.line && lastMove) {
          ...
          if (currentRow && checkLineWin(currentRow)) {
            ...
            if (winningPlayerId) {
              const winnerIndex = currentPlayers.findIndex(
                p => p.user_id === winningPlayerId
              );
              if (winnerIndex !== -1) {
        ...
        if (
          (timeExpired || markedFields.total === boardSize * boardSize) &&
          ...
          if (winners.length === 1 && winners[0]) {
            ...
            winners.length > 1 ||
            markedFields.total === boardSize * boardSize
          ) {
            setWinner(-1);
            emitGameEnd({
              winner: null,
  ...
  const updateBoardState = useCallback(
    (index: number, updates: Partial<GameBoardCell>) => {
      try {
        setBoardState(prev => {
          const newState = [...prev];
          if (newState[index]) {
