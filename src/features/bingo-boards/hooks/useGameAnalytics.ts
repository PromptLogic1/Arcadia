...
export const useGameAnalytics = (): UseGameAnalyticsReturn => {
  ...
  const trackEvent = useCallback((event: GameEvent) => {
    if (
      eventQueue.current.length >= ANALYTICS_CONSTANTS.EVENTS.MAX_QUEUE_SIZE
    ) {
      eventQueue.current = eventQueue.current.slice(
        -ANALYTICS_CONSTANTS.EVENTS.BATCH_SIZE
      );
    }
  ...
  const updateStats = useCallback(
    (
      ...
      setGameStats(prev => {
        const now = Date.now();
        const lastMoveTime =
          moveTimestamps.current[moveTimestamps.current.length - 1] || now;
        const moveTime = now - lastMoveTime;

        const updatedPlayerStats = players.reduce(
          (stats, player) => ({
            ...stats,
            [player.user_id]: {
              ...
              joinTime: new Date(
                player.joined_at || player.created_at || Date.now()
              ).getTime(),
              totalMoves:
                (prev.playerStats[player.user_id]?.totalMoves || 0) + 1,
        ...
        trackEvent({
          ...
          timestamp: now,
          sessionId:
            players[prev.moves % players.length]?.session_id || 'unknown',
  ...
  const trackMove = useCallback(
    (playerId: string, moveType: string, position: number) => {
      ...
      if (
        patterns.current.commonMoves.length <
          ANALYTICS_CONSTANTS.PATTERNS.MAX_PATTERN_LENGTH &&
      ...
      trackEvent({
        ...
        playerId,
        data: { moveType, position, playerId },
      });
    },
    [trackEvent]
  );
    ...
    (playerId: string | null) => {
      ...
      if (playerId !== null) {
        trackEvent({
          ...
          playerId,
          data: {
            duration: now - (gameStats.startTime || now),
            totalMoves: gameStats.moves,
          },
        });
      }
    },
    [gameStats.startTime, gameStats.moves, trackEvent]
  );
  ...
  const generateReport = useCallback((): GameReport => {
    ...
    return {
      ...
      recommendations: [],
      performanceIssues:
        analysis.averageMoveTime >
        ANALYTICS_CONSTANTS.PERFORMANCE.MAX_MOVE_LATENCY
  ...
  const measurePerformance = useCallback(() => {
    ...
    const frameTime = performanceMarks.current.frameTime
      ? (now - performanceMarks.current.frameTime) /
        Math.max(1, gameStats.moves)
      : 0;
