...
export const useLayout = (): UseLayout => {
  ...
  const layoutCache = useMemo(
    () => new Map<string, CachedLayout[keyof CachedLayout]>(),
    []
  );

  // Layout Berechnungen mit Konstanten
  const getGridLayout = useCallback(
    (size: number, isMobile: boolean): LayoutProperties['grid'] => {
      ...
      const layout = {
        columns: size,
        gap: isMobile
          ? LAYOUT_CONSTANTS.GRID.GAP_SM
          : LAYOUT_CONSTANTS.GRID.GAP_LG,
  ...
  const getResponsiveSpacing = useCallback(
    (base: number): LayoutProperties['spacing'] => {
  ...
  const getFluidTypography = useCallback(
    (min: number, max: number): LayoutProperties['typography'] => {
  ...
  const isSpacingLayout = (
    value: unknown
  ): value is LayoutProperties['spacing'] => {
    return (
      value !== null &&
      typeof value === 'object' &&
      'base' in value &&
  ...
  const isTypographyLayout = (
    value: unknown
  ): value is LayoutProperties['typography'] => {
    return (
      value !== null &&
      typeof value === 'object' &&
      'base' in value &&
  ...
  const isContainerLayout = (
    value: unknown
  ): value is LayoutProperties['container'] => {
    return (
      value !== null &&
      typeof value === 'object' &&
      'maxWidth' in value &&
  ...
  const emitLayoutChange = useCallback((breakpoint: Breakpoint) => {
    try {
      ...
      log.error('Error emitting layout change', error, {
        metadata: { hook: 'useLayout' },
      });
    }
  }, []);
