...
export interface UseSessionQueueModernReturn {
  ...
  setPlayerFormData: (data: UseSessionQueueModernReturn['playerFormData']) => void;
  ...
  updateQueueEntry: (entryId: string, updates: SessionQueueEntryUpdate) => Promise<void>;
  
  // Actions - Settings management
  setAutoRefreshEnabled: (enabled: boolean) => void;
  setQueueFilters: (filters: Partial<UseSessionQueueModernReturn['queueFilters']>) => void;
 ...
 */
export function useSessionQueueModern(sessionId: string): UseSessionQueueModernReturn {
  ...
  const playerStatusQuery = usePlayerQueueStatusQuery(userId, sessionId, !!userId);
  const playerPositionQuery = usePlayerQueuePositionQuery(userId, sessionId, !!userId);
  ...
  }, [uiState.autoRefreshEnabled, uiState.refreshInterval, sessionId, queueOperations]);
  ...
  const addToQueue = useCallback(async (playerData: PlayerQueueData) => {
    if (!authUser) {
  ...
  const acceptPlayer = useCallback(async (entryId: string) => {
    try {
  ...
  const rejectPlayer = useCallback(async (entryId: string) => {
    try {
  ...
  const updateQueueEntry = useCallback(async (entryId: string, updates: SessionQueueEntryUpdate) => {
    try {
  ...
  const updatePlayerFormField = useCallback((field: keyof NonNullable<SessionQueueState['playerFormData']>, value: string | number) => {
