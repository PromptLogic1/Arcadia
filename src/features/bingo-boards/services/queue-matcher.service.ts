...
export class QueueMatcherService {
  ...
  findMatches(queue: ExtendedQueueEntry[]): MatchGroup[] {
    ...
    for (const anchor of sortedQueue) {
      ...
      const minRequired = Math.max(anchor.preferences?.minPlayers || this.MIN_PLAYERS, this.MIN_PLAYERS);
      ...
      if (totalPlayers >= minRequired) {
        ...
        const finalPlayers = [anchor, ...compatiblePlayers.slice(0, maxAllowed - 1)];
        
        matches.push({
          entries: finalPlayers,
          boardId: anchor.board_id,
          avgWaitTime: finalPlayers.reduce((sum, p) => sum + p.waitTime, 0) / finalPlayers.length,
  ...
  private areCompatible(anchor: ExtendedQueueEntry, candidate: ExtendedQueueEntry): boolean {
  ...
  private getSkillDifference(player1: ExtendedQueueEntry, player2: ExtendedQueueEntry): number {
  ...
  calculateMatchQuality(match: MatchGroup): number {
    ...
    if (match.avgWaitTime > 120) { // 2 minutes
      quality += 20;
    }
  ...
  estimateWaitTime(
    ...
    const compatibleCount = currentQueue.filter(e => 
      e.id !== entry.id && this.areCompatible(entry, e)
    ).length;
