...
import type { WinPattern, WinDetectionResult } from '../types/win-patterns.types';
...
export class WinDetectionService {
  ...
  detectWin(boardState: BoardCell[]): WinDetectionResult {
    ...
    const fullHousePattern = this.checkFullHouse(markedPositions, boardState.length);
    
    detectedPatterns.push(...rowPatterns, ...columnPatterns, ...diagonalPatterns);
    ...
    const winningCells = [...new Set(detectedPatterns.flatMap(p => p.positions))];
  ...
  private checkFullHouse(marked: number[], totalCells: number): WinPattern | null {
  ...
  checkLetterT(marked: number[]): WinPattern | null {
    ...
    const tPattern = [
      ...
      ...Array.from({ length: this.boardSize - 1 }, (_, i) => 
        Math.floor(this.boardSize / 2) + ((i + 1) * this.boardSize)
      )
    ];
