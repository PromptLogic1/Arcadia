...
class BingoEngine {
  ...
  validateGameState(state: GameState): { valid: boolean; errors: string[] } {
    ...
    if (state.currentState.length !== expectedCells) {
      errors.push(`Invalid board size: expected ${expectedCells} cells, got ${state.currentState.length}`);
    }
...
describe('BingoEngine', () => {
  ...
  function createInitialGameState(size = 25): GameState {
    const cells: BoardCell[] = Array.from({ length: size }, (_, i) => ({
      text: `Cell ${i}`,
      colors: null,
      completed_by: [],
      blocked: false,
  ...
  describe('Game State Validation', () => {
    ...
    test('should detect invalid board size', () => {
      ...
      expect(validation.errors).toContain('Invalid board size: expected 25 cells, got 20');
    });
  ...
  describe('Complex Game Scenarios', () => {
    test('should handle multiple simultaneous wins', () => {
      ...
      const setupMoves = [
        0, 1, 2, 3,    // Row 1 (missing 4)
        0, 5, 10, 15   // Column 1 (missing 20, but 0 is shared)
      ];
    ...
    test('should maintain game state consistency through complex interactions', () => {
      ...
      moves.forEach(move => {
        const result = engine.processMove(gameState, move.pos, move.player, move.action);
        gameState = result.newState;
  ...
  describe('Edge Cases', () => {
    ...
    test('should handle all cells being marked (full house)', () => {
      ...
      for (let i = 0; i < 25; i++) {
        ...
        if (i === 24) {
          expect(result.winDetected).toBe(true);
          const fullHouse = result.winPatterns?.find(p => p.type === 'full-house');
