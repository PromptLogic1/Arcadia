...
export async function triggerNotification(
  ...
  options: {
    ...
    blockedUsers?: string[];
    emailQueue?: (email: any) => void;
    pushService?: (push: any) => void;
  } = {}
...
export async function formatNotificationContent(notification: any): Promise<NotificationContent> {
  ...
  switch (type) {
    ...
    case 'upvote_batch':
      const actorNames = notification.actors?.slice(0, 3).map((a: any) => a.username).join(', ');
