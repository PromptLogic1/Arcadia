...
export type TrustLevel = 'new' | 'basic' | 'regular' | 'trusted' | 'moderator' | 'banned' | 'vip';
...
const rateLimitStore = new Map<string, { count: number; reset: number; window?: number }>();

export async function enforceRateLimits(config: RateLimitConfig): Promise<RateLimitResult> {
