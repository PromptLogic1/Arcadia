...
export function rankSearchResults(
  ...
  return results.sort((a, b) => {
    ...
    // Secondary sort: prefer title matches over content matches for ties
    const aHasTitle = a.highlights.title && a.highlights.title.length > 0;
    const bHasTitle = b.highlights.title && b.highlights.title.length > 0;
    ...
    // Tertiary sort by recency for ties
    const aTime = a.discussion.created_at
      ? new Date(a.discussion.created_at).getTime()
      : 0;
    const bTime = b.discussion.created_at
...
export function advancedSearch(
  ...
  if (filters.sortBy === 'relevance' && filters.searchTerm) {
    ...
    return sortedDiscussions
      .map(discussion => results.find(r => r.discussion.id === discussion.id))
