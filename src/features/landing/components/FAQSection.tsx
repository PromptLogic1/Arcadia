...
const FAQSection: React.FC = () => {
  return (
    ...
    >
      <FloatingElements variant="circuits" count={15} speed="slow" color="purple" repositioning={true} />
      <section className="relative z-20" aria-labelledby="faq-heading">
        <div className="container mx-auto flex flex-col items-center px-4">
          <h2 id="faq-heading" className="mb-16 text-center">
            <NeonText variant="gradient" className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl 2xl:text-8xl">FAQ</NeonText>
          </h2>
            ...
            <Accordion type="single" collapsible>
              {faqs.map(faq => (
                <AccordionItem
                  key={faq.id}
                  value={faq.id}
                  className="mb-6 rounded-lg cyber-card border-cyan-500/30 shadow-lg shadow-cyan-500/10"
                >
                  <AccordionTrigger 
                    className="px-8 py-6 text-xl font-semibold text-cyan-200 hover:text-cyan-300 transition-colors [&[data-state=open]]:text-cyan-300"
                    aria-expanded={false}
                  >
                    {faq.question}
                  </AccordionTrigger>
                  <AccordionContent className="px-8 pb-6 text-cyan-100/90 leading-relaxed text-base">
                    {faq.answer}
                  </AccordionContent>
                </AccordionItem>
              ))}
