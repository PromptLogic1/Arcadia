...
export class SpeedrunTimer {
  ...
  formatTime(includeMilliseconds = true): string {
    const elapsed = this.state.isRunning
      ? this.getElapsedTime()
  ...
  validateIntegrity(): TimerValidation {
    ...
    if (this.snapshots.length > 1) {
      for (let i = 1; i < this.snapshots.length; i++) {
        ...
        if (
          curr.elapsedTime < prev.elapsedTime &&
          curr.state === 'running' &&
          prev.state === 'running'
        ) {
        ...
        if (
          Math.abs(timeDiff - elapsedDiff) > 100 &&
          curr.state === 'running' &&
          prev.state === 'running'
        ) {
...
describe('Speedrun Timer', () => {
  ...
  describe('Timer Validation', () => {
    ...
    test('should detect suspiciously fast times', () => {
      ...
      expect(validation.suspiciousPatterns).toContain(
        'Suspiciously fast completion time'
      );
    });
