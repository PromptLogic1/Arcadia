...
export type QuestStatus = 'draft' | 'published' | 'featured' | 'archived' | 'maintenance'
...
export interface QuestStory {
  ...
  introduction: string
  conclusion: string
  narrative_style: 'adventure' | 'mystery' | 'sci-fi' | 'fantasy' | 'educational'
}
...
export type QuestProgressStatus = 'not_started' | 'in_progress' | 'completed' | 'abandoned' | 'locked'
...
// Constants
export const QUEST_THEMES: Record<QuestTheme, { name: string; icon: string; color: string }> = {
  space_exploration: { name: 'Space Exploration', icon: '🚀', color: '#1E3A8A' },
  medieval_fantasy: { name: 'Medieval Fantasy', icon: '🏰', color: '#7C2D12' },
  cyberpunk: { name: 'Cyberpunk', icon: '🤖', color: '#EC4899' },
  detective_mystery: { name: 'Detective Mystery', icon: '🔍', color: '#374151' },
