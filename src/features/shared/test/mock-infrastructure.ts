...
export function createMockFetch() {
  const responses = new Map<string, { status: number; data: any; delay?: number }>();
  const callHistory: Array<{ url: string; options?: RequestInit; timestamp: number }> = [];
...
export class NetworkFailureSimulator {
  private failurePatterns = new Map<string, { type: 'timeout' | 'error' | 'intermittent'; rate?: number }>();
  
  addFailurePattern(urlPattern: string, type: 'timeout' | 'error' | 'intermittent', rate = 1): void {
  ...
  shouldFail(url: string): { fail: boolean; type?: 'timeout' | 'error' } {
    for (const [pattern, config] of this.failurePatterns) {
      if (url.includes(pattern)) {
        const shouldFail = config.type === 'intermittent' 
...
export class ChaosHelper {
  ...
  async maybeInjectChaos<T>(operation: () => Promise<T>): Promise<T> {
    ...
    // Random latency
    const latency = this.chaosConfig.latencyMs.min + 
      Math.random() * (this.chaosConfig.latencyMs.max - this.chaosConfig.latencyMs.min);
