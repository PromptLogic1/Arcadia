...
function generateActivityDescription(
  ...
  if (!data) {
    return (
      defaultDescriptions[activityType] || `Performed activity: ${activityType}`
    );
  }
...
function calculatePointsEarned(
  ...
  if (
    ...
    (activityType === 'board_create' ||
      activityType === 'board_join' ||
      activityType === 'board_complete')
  ) {
...
describe('Activity Tracker', () => {
  describe('Activity Description Generation', () => {
    it('should generate default descriptions without data', () => {
      ...
      activities.forEach((activity, index) => {
        expect(generateActivityDescription(activity)).toBe(
      ...
      const testCases = [
        {
          ...
          data: {
            board_id: '1',
            board_title: 'Epic Adventure Board',
        ...
        },
        {
          ...
          data: {
            challenge_id: '4',
            challenge_title: 'Photo Hunt Challenge',
            language: 'python',
        ...
        },
        {
          ...
          data: {
            discussion_id: 5,
            title: 'Best strategies for speedruns?',
            game: 'Apex Legends',
          } as ActivityData,
        ...
        },
        {
          ...
          data: {
            comment_id: 6,
            discussion_id: 5,
            discussion_title: 'Game Tips Thread',
            content_preview: 'Great tips!',
          } as ActivityData,
      ...
      const testCases = [
        {
          ...
          data: {
            ...
            game_type: 'World of Warcraft' as const,
            difficulty: 'medium' as const,
          } as ActivityData, // Missing board_title
  ...
  describe('Batch Processing', () => {
    ...
    it('should process activities in chunks efficiently', () => {
      ...
      // Process in batches
      const batches: Array<
        Array<{ type: ActivityType; data: { index: number } }>
      > = [];
      ...
      const processedCount = batches.reduce(
  ...
  describe('Time-Series Data', () => {
    ...
    it('should handle activities across different time periods', () => {
      ...
      const dailyPoints = Object.entries(dailyActivities).map(
        ([day, dayActivities]) => ({
          day,
          points: dayActivities.reduce(
            (sum, a) => sum + calculatePointsEarned(a.type),
            0
          ),
        })
      );
      ...
      const todayPoints = dailyPoints.find(
        d => d.day === now.toISOString().split('T')[0]
      );
    ...
    it('should aggregate activities by hour for recent activity', () => {
      const now = new Date();
      const recentActivities: TimedActivity[] = Array.from(
        { length: 24 },
        (_, hour) => ({
          type: 'comment_create' as ActivityType,
          ...
          data: {
            comment_id: hour,
            discussion_id: 1,
            discussion_title: `Discussion ${hour}`,
            content_preview: 'Comment',
          } as ActivityData,
        })
      );

      // Group by hour
      const hourlyGroups = recentActivities.reduce(
        (acc, activity) => {
          const hour = Math.floor(
            (now.getTime() - activity.timestamp.getTime()) / (60 * 60 * 1000)
          );
  ...
  describe('Edge Cases', () => {
    it('should handle unknown activity types', () => {
      ...
      expect(generateActivityDescription(unknownType)).toBe(
        'Performed activity: unknown_activity'
      );
