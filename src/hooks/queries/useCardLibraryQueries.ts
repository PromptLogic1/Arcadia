...
import type { BingoCard, GameCategory, Difficulty as _Difficulty } from '@/types';

/**
 * Get public cards with filtering and pagination
 */
export function useCardLibraryPublicCardsQuery(filters: CardLibraryFilters, page = 1, enabled = true) {
  return useQuery({
 ...
 */
export function useFeaturedCollectionsQuery(gameType: GameCategory, enabled = true) {
  return useQuery({
...
export function useCreateBulkCardsMutation() {
  ...
  return useMutation({
    mutationFn: (cards: Omit<BingoCard, 'id' | 'created_at' | 'updated_at' | 'votes'>[]) =>
    ...
    onSuccess: (result, _variables) => {
      ...
      notifications.success(`Created ${result.cards.length} cards successfully!`);
...
export function useShuffleCardsMutation() {
  ...
  return useMutation({
    ...
    onError: (error) => {
      const message = error instanceof Error ? error.message : 'Failed to shuffle cards';
      notifications.error(message);
    },
  });
}