...
interface BingoCardsState {
  ...
  // Service Methods
  initializeCards: (userId: string, gameCategory: GameCategory) => Promise<BingoCard[]>
  getCardById: (cardId: string) => Promise<BingoCard | null>
  createCard: (cardData: CreateBingoCardDTO, userId: string) => Promise<BingoCard | null>
  updateCardService: (cardId: string, updates: Partial<BingoCard>, userId: string, gameCategory: GameCategory) => Promise<BingoCard | null>
  deleteCard: (cardId: string, userId: string, gameCategory: GameCategory) => Promise<boolean>
  voteCard: (cardId: string) => Promise<boolean>
  getCardsByGameCategory: (gameCategory: GameCategory) => Promise<BingoCard[]>
  filterCards: (filters: { gameCategory?: GameCategory; cardType?: string; difficulty?: DifficultyLevel; searchTerm?: string }) => Promise<BingoCard[]>
  ...
  initializePublicCards: (gameCategory: GameCategory, page?: number) => Promise<void>
  filterPublicCards: (filters: FilterOptions, gameCategory: GameCategory, page?: number) => Promise<void>
}

export const useBingoCardsStore = create<BingoCardsState>()(
  devtools(
    (set, get) => ({
      // Initial state
      cards: [],
      ...
      // Basic Actions
      setLoading: (loading) =>
        set({ loading }, false, 'bingoCards/setLoading'),

      setError: (error) =>
        set({ error }, false, 'bingoCards/setError'),
      ...
      // Service Methods
      initializeCards: async (userId: string, gameCategory: GameCategory): Promise<BingoCard[]> => {
        try {
          get().setLoading(true)
      ...
      createCard: async (cardData: CreateBingoCardDTO, userId: string): Promise<BingoCard | null> => {
        try {
          get().setLoading(true)
      ...
      updateCardService: async (cardId: string, updates: Partial<BingoCard>, userId: string, gameCategory: GameCategory): Promise<BingoCard | null> => {
        try {
          get().setLoading(true)
      ...
      deleteCard: async (cardId: string, userId: string, gameCategory: GameCategory): Promise<boolean> => {
        try {
          get().setLoading(true)
      ...
      getCardsByGameCategory: async (gameCategory: GameCategory): Promise<BingoCard[]> => {
        try {
          get().setLoading(true)
      ...
      initGridCards: async (layoutIds: string[]): Promise<void> => {
        try {
          ...
          if (realCardIds.length > 0) {
            ...
            gridCards = layoutIds.map(() => ({ ...DEFAULT_BINGO_CARD })) as BingoCard[]
          }
      ...
      initializePublicCards: async (gameCategory: GameCategory, page = 1): Promise<void> => {
        try {
          get().setLoading(true)
          get().setError(null)
      ...
      filterPublicCards: async (filters: FilterOptions, gameCategory: GameCategory, page = 1): Promise<void> => {
        try {
          get().setLoading(true)
          get().setError(null)
          get().clearPublicCards()
...
// Selectors
export const useBingoCards = () => useBingoCardsStore((state) => ({
  cards: state.cards,
...
export const useBingoCardsActions = () => useBingoCardsStore((state) => ({
