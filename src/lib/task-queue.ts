  ...
  try {
    ...
    switch (task.type) {
      case 'bingo-generation':
        result = await generateBingoBoard(
          task.payload as BingoGenerationPayload
        );
        break;
      case 'code-execution':
...
async function generateBingoBoard(
  _payload: BingoGenerationPayload
): Promise<BingoGenerationResult> {
