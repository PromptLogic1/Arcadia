...
export const gameStateService = {
  async getSessionState(sessionId: string): Promise<{ session: BingoSession | null; error?: string }> {
    ...
    try {
      ...
      return { session: data as unknown as BingoSession };
    } catch (error) {
  ...
  async markCell(sessionId: string, data: MarkCellData): Promise<{ boardState: BoardCell[] | null; version: number; error?: string }> {
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to mark cell';
      return { boardState: null, version: 0, error: message };
    }
  },

  async completeGame(sessionId: string, data: CompleteGameData): Promise<{ success: boolean; error?: string }> {
    try {
  ...
  async getGameResults(sessionId: string): Promise<{ results: GameResult[] | null; error?: string }> {
    const supabase = createClient();
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'Failed to load game results';
      return { results: null, error: message };
    }
  },

  async getBoardState(sessionId: string): Promise<{ boardState: BoardCell[] | null; version: number; error?: string }> {
    try {
      const response = await fetch(`/api/bingo/sessions/${sessionId}/board-state`);
      ...
      const message = error instanceof Error ? error.message : 'Failed to get board state';
      return { boardState: null, version: 0, error: message };
    }
  }
};