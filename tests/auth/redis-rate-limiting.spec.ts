...
test.describe('Redis Rate Limiting Integration', () => {
  ...
  test.beforeAll(async () => {
    ...
    if (!process.env.USE_REDIS_TESTS) {
      test.skip(true, 'Redis integration tests skipped - set USE_REDIS_TESTS=true to enable');
    }
  });

  test.describe('Login Rate Limiting', () => {
    test('should enforce rate limiting with Redis persistence', async ({ page }) => {
      ...
      for (let i = 1; i <= maxAttempts + 1; i++) {
        ...
        if (i <= maxAttempts) {
          ...
          if (errorMessage?.match(/rate.*limit|too.*many/i)) {
            ...
            console.log(`✓ Rate limiting enforced after ${maxAttempts} attempts`);
          } else {
  ...
  test.describe('Multi-Algorithm Rate Limiting', () => {
    test('should enforce different algorithms for different endpoints', async ({ page }) => {
      ...
      const resetSuccess = await page.getByText(/check.*email|sent.*link/i).isVisible().catch(() => false);
      const resetError = await getAuthErrorMessage(page);
      
      console.log(`Login rate limited: ${!!loginRateLimited}, Reset success: ${resetSuccess}, Reset error: ${resetError}`);
