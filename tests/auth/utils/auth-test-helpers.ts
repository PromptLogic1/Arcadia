 ...
 */
export async function getAuthStoreState(page: Page): Promise<TestAuthState | null> {
  return await page.evaluate(() => {
    const testWindow = window as TestWindow;
 ...
 */
export async function waitForAuthStore(page: Page, timeout = 5000): Promise<void> {
...
export async function checkAuthAccessibility(
  ...
  const inputsWithoutLabels = await page.evaluate((sel?: string) => {
    ...
    const inputs = root.querySelectorAll('input:not([type="hidden"]):not([type="submit"])');
    let count = 0;
