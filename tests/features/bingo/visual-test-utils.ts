    ...
    for (const state of states) {
      ...
      if (state.regions) {
        for (const region of state.regions) {
          const regionScreenshot = await this.page
...
export class AnimationTester {
   ...
   */
  private async getBoardClip(): Promise<{
    x: number;
    y: number;
    width: number;
    height: number;
...
export class AccessibilityVisualTester {
  ...
  async testHighContrast(): Promise<Map<string, Buffer>> {
    ...
    screenshots.set(
      'high-contrast-empty',
      await this.page.getByTestId('bingo-grid').screenshot()
    );
