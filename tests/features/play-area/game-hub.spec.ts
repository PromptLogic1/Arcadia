...
test.describe('Play Area Game Hub', () => {
  ...
  test.describe('Game Discovery and Browsing', () => {
    test('should display play area hub with active sessions', async ({ authenticatedPage }) => {
      // Wait for page to load
      await expect(authenticatedPage.locator('h1')).toContainText('Play Area');
      
      // Check main navigation elements
      await expect(authenticatedPage.getByRole('button', { name: /host session/i })).toBeVisible();
      await expect(authenticatedPage.getByRole('button', { name: /join by code/i })).toBeVisible();
      
      // Check stats cards
      await expect(authenticatedPage.locator('[data-testid="active-players-card"]', { hasText: 'Active Players' })).toBeVisible();
      await expect(authenticatedPage.locator('[data-testid="recent-activity-card"]', { hasText: 'Recent Activity' })).toBeVisible();
      
      // Check session tabs
      await expect(authenticatedPage.getByRole('tab', { name: /public sessions/i })).toBeVisible();
      await expect(authenticatedPage.getByRole('tab', { name: /join session/i })).toBeVisible();
    });

    test('should show empty state when no sessions available', async ({ authenticatedPage }) => {
      ...
      // Check empty state
      await expect(authenticatedPage.locator('[data-testid="empty-sessions"]')).toBeVisible();
      await expect(authenticatedPage.locator('text=No Active Sessions')).toBeVisible();
      await expect(authenticatedPage.locator('text=Be the first to host a gaming session!')).toBeVisible();
    });

    test('should display session cards with proper information', async ({ authenticatedPage }) => {
      // Mock sessions data
      await mockApiResponse(authenticatedPage, '**/api/sessions**', {
        status: 200,
        body: {
    ...
    test('should handle session filters correctly', async ({ authenticatedPage }) => {
    ...
    test('should refresh sessions list', async ({ authenticatedPage }) => {
      ...
      await expect(refreshButton.locator('[data-testid="loading-spinner"]')).toBeVisible({ timeout: 1000 });
      
      // Should complete refresh
      await expect(refreshButton.locator('[data-testid="loading-spinner"]')).not.toBeVisible({ timeout: 5000 });
    });
  });

  test.describe('Session Creation Flow', () => {
    test('should open host session dialog', async ({ authenticatedPage }) => {
      await authenticatedPage.getByRole('button', { name: /host session/i }).click();
      
      // Check dialog opens
      const dialog = authenticatedPage.locator('[data-testid="host-session-dialog"]');
    ...
    test('should create session with valid inputs', async ({ authenticatedPage }) => {
      ...
      await expect(authenticatedPage).toHaveURL(/\/play-area\/session\/new-session-123/);
    });

    test('should handle session creation errors', async ({ authenticatedPage }) => {
      ...
      await expect(authenticatedPage.locator('[role="alert"]')).toContainText(/failed to create/i);
    });
  });

  test.describe('Join Session Flow', () => {
    test('should open join by code dialog', async ({ authenticatedPage }) => {
      await authenticatedPage.getByRole('button', { name: /join by code/i }).click();
      
      // Check dialog opens
      const dialog = authenticatedPage.locator('[data-testid="join-session-dialog"]');
      await expect(dialog).toBeVisible();
    ...
    test('should join session with valid code', async ({ authenticatedPage }) => {
      // Switch to join session tab first
      await authenticatedPage.getByRole('tab', { name: /join session/i }).click();
      ...
      // Submit
      await authenticatedPage.getByRole('button', { name: /join session/i }).click();
      
      // Should process join attempt (implementation may vary)
      await waitForNetworkIdle(authenticatedPage);
    });

    test('should join session directly from session card', async ({ authenticatedPage }) => {
      ...
      const joinButton = authenticatedPage.locator('[data-testid="join-session-button"]').first();
      await joinButton.click();
      
      // Should redirect to session
      await expect(authenticatedPage).toHaveURL(/\/play-area\/session\/joinable-session/);
    });
  });
  ...
  test.describe('Performance and Accessibility', () => {
    ...
    test('should be accessible', async ({ authenticatedPage }) => {
      ...
      const hostButton = authenticatedPage.getByRole('button', { name: /host session/i });
      ...
      const joinButton = authenticatedPage.getByRole('button', { name: /join by code/i });
      await expect(joinButton).toBeFocused();
    });

    test('should handle large session lists efficiently', async ({ authenticatedPage }) => {
      // Mock many sessions
  ...
  test.describe('Real-time Updates', () => {
    test('should update session list in real-time', async ({ authenticatedPage }) => {
      // Initial empty state
      await mockApiResponse(authenticatedPage, '**/api/sessions**', {
        status: 200,
        body: { sessions: [], total: 0 }
      ...
      await expect(authenticatedPage.locator('[data-testid="session-card-new-session"]')).toBeVisible({
        timeout: 3000
      });
    });

    test('should update player counts in real-time', async ({ authenticatedPage }) => {
      ...
      const sessionCard = authenticatedPage.locator('[data-testid="session-card-updating-session"]');
  ...
  test.describe('Error Handling', () => {
    test('should handle API errors gracefully', async ({ authenticatedPage }) => {
      ...
      await expect(authenticatedPage.locator('text=Failed to load sessions')).toBeVisible();
    });

    test('should handle network disconnection', async ({ authenticatedPage, context }) => {
      ...
      await expect(authenticatedPage.locator('[data-testid="connection-status"]')).toContainText(/offline/i, {
        timeout: 5000
      });
      ...
      // Should recover
      await refreshButton.click();
      await expect(authenticatedPage.locator('[data-testid="connection-status"]')).toContainText(/online/i, {
        timeout: 5000
      });
    });
  });
