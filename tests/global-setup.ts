...
async function globalSetup(playwrightConfig: FullConfig): Promise<void> {
  ...
  if (envResult.error) {
    console.warn('⚠️  Warning: Could not load .env.test file:', envResult.error.message);
  ...
  try {
    ...
    if (!appReady) {
      ...
      console.error('');
      console.error('💡 If you want to run tests without a dev server, use CI mode:');
      console.error('   CI=true npm run test:e2e');
      throw new Error('❌ Application failed to start within timeout - dev server not running');
    }
