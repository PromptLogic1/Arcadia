...
test.describe('API Error Response Handling', () => {
  test.describe('HTTP Status Code Handling', () => {
    ...
    test('should handle 404 Not Found for API endpoints', async ({
      page,
      context,
    }) => {
      await mockApiResponse(page, '**/api/**', {
      ...
      const dataActions = page.locator(
        'button:has-text("Load"), button:has-text("Fetch"), button:has-text("Get")'
      );
    ...
    test('should handle 500 Internal Server Error', async ({
      ...
      if ((await button.count()) > 0) {
        ...
        await expect(
          page.locator('text="Database connection failed"')
        ).not.toBeVisible();
  ...
  test.describe('Error Message Security', () => {
    test('should not expose sensitive data in error responses', async ({
      ...
      await expect(
        page.locator('text="super-secret-password"')
      ).not.toBeVisible();
    ...
    test('should sanitize error messages from external APIs', async ({
      page,
      context,
    }) => {
      ...
      const externalButton = page.locator(
        'button:has-text("External"), button:has-text("Third-party")'
      );
  ...
  test.describe('Error Context and Recovery', () => {
    test('should provide contextual error messages', async ({
      ...
      const userButton = page.locator(
        'button:has-text("User"), button:has-text("Profile")'
      );
      if ((await userButton.count()) > 0) {
        await userButton.click();
        await page.waitForTimeout(500);

        const userErrorMessages = page.locator(
          'text="User not found", text="Profile not available"'
        );
      ...
      const gameButton = page.locator(
        'button:has-text("Game"), button:has-text("Play")'
      );
      if ((await gameButton.count()) > 0) {
        await gameButton.click();
        await page.waitForTimeout(500);

        const gameErrorMessages = page.locator(
          'text="Game not found", text="Game not available"'
        );
    ...
    test('should handle progressive error states', async ({
      page,
      context,
    }) => {
      let attemptCount = 0;
  ...
  test.describe('API Error Logging', () => {
    ...
    test('should include request context in error logs', async ({ page }) => {
      ...
      if (networkRequests.length > 0) {
        ...
        if (
          failedRequest &&
          typeof failedRequest === 'object' &&
          'url' in failedRequest &&
          'method' in failedRequest
        ) {
  ...
  test.describe('Error Persistence', () => {
    test('should clear errors on successful retry', async ({
      ...
      if ((await button.count()) > 0) {
        ...
        const errorMessage = page.locator(
          'text="error", text="failed", text="wrong"'
        );
        ...
        if ((await retryButton.count()) > 0) {
          ...
          const successMessage = page.locator(
            'text="success", text="complete", text="loaded"'
          );
    ...
    test('should not persist errors across page navigation', async ({
      page,
      context,
    }) => {
      await mockApiResponse(page, '**/api/**', {
      ...
      const errorMessages = page.locator(
        'text="Server Error", text="error occurred"'
      );
