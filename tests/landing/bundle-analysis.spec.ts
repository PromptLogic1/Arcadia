...
async function analyzeDetailedBundles(
  ...
  resources.forEach((resource: ResourceEntry) => {
    ...
    if (resource.initiatorType === 'script' || url.includes('.js')) {
      ...
      /\.(jpg|jpeg|png|webp|svg|gif)(\?|$)/i.test(url)
    ) {
  ...
  const compressionRatio =
    totalDecoded > 0 ? totalTransferred / totalDecoded : 1;
...
function findDuplicateResources(
...
): Array<{ name: string; count: number; totalSize: number }> {
  ...
  resources.forEach(resource => {
    ...
    if (filename) {
      const existing = resourceCounts.get(filename) || {
        count: 0,
        totalSize: 0,
      };
...
test.describe('Bundle Analysis & Performance Monitoring', () => {
  ...
  test('should meet bundle size budgets @bundle @performance @critical', async ({
    page,
  }) => {
    await page.goto('/');
    ...
    console.log(
      `Compression Ratio: ${(bundleMetrics.compressionRatio * 100).toFixed(1)}%`
    );

    // Assert bundle size thresholds
    expect(bundleMetrics.mainBundle).toBeLessThan(BUNDLE_THRESHOLDS.mainBundle);
    expect(bundleMetrics.vendorBundle).toBeLessThan(
      BUNDLE_THRESHOLDS.vendorBundle
    );
    ...
    const oversizedChunks = bundleMetrics.chunks.filter(
      chunk => chunk.size > BUNDLE_THRESHOLDS.individualChunk
    );
  ...
  test('should optimize asset loading and compression @bundle @optimization', async ({
    page,
  }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
    ...
    Object.entries(bundleMetrics.resourceBreakdown).forEach(
      ([type, resources]) => {
        const totalSize = resources.reduce((sum, r) => sum + r.size, 0);
        console.log(
          `${type}: ${resources.length} files, ${(totalSize / 1024).toFixed(2)}KB`
        );

        // Check against budgets
        const budget =
          ASSET_BUDGETS[type as keyof typeof ASSET_BUDGETS] ||
          ASSET_BUDGETS.other;
    ...
    if (bundleMetrics.duplicates.length > 0) {
      ...
      bundleMetrics.duplicates.forEach(dup => {
        console.log(
          `  - ${dup.name}: ${dup.count} copies, ${(dup.totalSize / 1024).toFixed(2)}KB total`
        );
      });
    }
  ...
  test('should optimize critical path resources @bundle @critical-path', async ({
    page,
  }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
    ...
    bundleMetrics.criticalPath.forEach((resource, index) => {
      console.log(
        `  ${index + 1}. ${resource.name}: ${(resource.size / 1024).toFixed(2)}KB (${resource.loadTime.toFixed(0)}ms)`
      );
    });

    // Critical path should be optimized
    const criticalSize = bundleMetrics.criticalPath.reduce(
      (sum, r) => sum + r.size,
      0
    );
    ...
    const slowCriticalResources = bundleMetrics.criticalPath.filter(
      r => r.loadTime > 1000
    );
    expect(slowCriticalResources.length).toBeLessThanOrEqual(2);
  });

  test('should implement effective code splitting @bundle @code-splitting', async ({
    page,
  }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
    ...
    const jsChunks = bundleMetrics.chunks.filter(chunk =>
      chunk.name.includes('.js')
    );
    ...
    const vendorChunks = jsChunks.filter(
      chunk =>
  ...
  test('should optimize image loading and formats @bundle @images', async ({
    page,
  }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
    ...
    if (images.length > 0) {
      ...
      console.log(
        `Average image size: ${(averageImageSize / 1024).toFixed(2)}KB`
      );

      // Check image formats
      const imageFormats = images.reduce(
        (formats, img) => {
      ...
      console.log(
        `Modern format usage: ${(modernFormatRatio * 100).toFixed(1)}%`
      );

      // Check for oversized images
      const oversizedImages = images.filter(
        img => img.size > BUNDLE_THRESHOLDS.imageOptimization
      );
      ...
      // Assertions
      expect(averageImageSize).toBeLessThan(
        BUNDLE_THRESHOLDS.imageOptimization
      );
  ...
  test('should implement effective font loading strategy @bundle @fonts', async ({
    page,
  }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
    ...
    if (fonts.length > 0) {
      ...
      // Check font formats
      const fontFormats = fonts.reduce(
        (formats, font) => {
  ...
  test('should monitor bundle size regression over time @bundle @regression', async ({
    page,
  }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
    ...
    const bundleReport = {
      ...
      breakdown: {
        javascript: bundleMetrics.resourceBreakdown.scripts.reduce(
          (sum, s) => sum + s.size,
          0
        ),
        css: bundleMetrics.resourceBreakdown.stylesheets.reduce(
          (sum, s) => sum + s.size,
          0
        ),
        images: bundleMetrics.resourceBreakdown.images.reduce(
          (sum, i) => sum + i.size,
          0
        ),
        fonts: bundleMetrics.resourceBreakdown.fonts.reduce(
          (sum, f) => sum + f.size,
          0
        ),
        other: bundleMetrics.resourceBreakdown.other.reduce(
          (sum, o) => sum + o.size,
          0
        ),
      },
    ...
    expect(bundleMetrics.mainBundle).toBeLessThan(
      regressionThresholds.mainBundle
    );
    expect(bundleMetrics.vendorBundle).toBeLessThan(
      regressionThresholds.vendorBundle
    );
    expect(bundleMetrics.cssBundle).toBeLessThan(
      regressionThresholds.cssBundle
    );
    expect(bundleMetrics.totalSize).toBeLessThan(
      regressionThresholds.totalSize
    );
  ...
  test('should validate tree shaking and dead code elimination @bundle @tree-shaking', async ({
    page,
  }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
    ...
    const jsAssets = bundleMetrics.assets.filter(
      asset => asset.type === 'javascript'
    );

    console.log('\n🌳 Tree Shaking Analysis:');

    // Check for common signs of ineffective tree shaking
    const suspiciouslyLargeChunks = jsAssets.filter(
      asset => asset.size > 150 * 1024 && !asset.name.includes('vendor')
    );
    ...
    console.log(
      `Vendor/App ratio: ${(vendorRatio * 100).toFixed(1)}% vendor, ${((1 - vendorRatio) * 100).toFixed(1)}% app`
    );
