...
export async function collectPerformanceMetrics(
  ...
  return await page.evaluate(() => {
    ...
    if (fidEntries.length > 0) {
      const fidEntry = fidEntries[0] as unknown as {
    ...
    try {
      ...
      const inpEntries = performance.getEntriesByType(
        'event'
      ) as PerformanceEntry[];
    ...
    const totalSize = resources.reduce((sum, resource) => {
      return (
        sum +
        ((resource as unknown as { transferSize?: number }).transferSize || 0)
      );
    }, 0);

    const cachedRequests = resources.filter(
      resource =>
        (resource as unknown as { transferSize?: number }).transferSize === 0
    ).length;
    ...
    const timeToInteractive = navTiming
    ...
    const speedIndex =
      firstContentfulPaint * 0.8 + largestContentfulPaint * 0.2;
