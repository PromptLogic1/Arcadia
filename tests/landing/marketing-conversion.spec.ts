...
test.describe('Marketing Tag Verification', () => {
  test('should load all required marketing tags @marketing', async ({
    page,
  }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');

    const expectedTags = ['google-analytics', 'google-tag-manager'] as const;
    ...
    results.forEach(result => {
      console.log(
        `  ${result.tag}: ${result.loaded ? '✓' : '✗'} ${result.error || ''}`
      );
    });
  ...
  test('should not load tracking in privacy mode', async ({
    page,
    context,
  }) => {
    // Set Do Not Track
    ...
    if (hasTracking) {
      console.warn(
        'Tracking loaded despite DNT header - verify limited tracking mode'
      );
    }
  });
});
  ...
  test('should track video engagement if present', async ({ page }) => {
    ...
    if ((await video.count()) > 0) {
      ...
      const videoEvents = events.filter(
        e => e.name.includes('video') || e.category === 'video'
      );

      expect(videoEvents.length).toBeGreaterThan(0);
      console.log(
        'Video events tracked:',
        videoEvents.map(e => e.name)
      );
    }
  });
});
...
test.describe('UTM Parameter Tracking', () => {
  ...
  test('should track social media campaign parameters', async ({ page }) => {
    ...
    const campaignUrl = CAMPAIGN_ROUTES.socialCampaign(
      'twitter',
      'product-launch'
    );
    await page.goto(campaignUrl);
...
test.describe('A/B Testing Scenarios', () => {
  test('should track A/B test variant exposure', async ({ page }) => {
    ...
    const experimentEvent = await analytics.waitForEvent(
      'experiment_viewed',
      3000
    );
  ...
  test('should maintain consistent variant assignment', async ({ page }) => {
    ...
    const experimentCookies = cookies.filter(
      c => c.name.includes('ab_') || c.name.includes('experiment')
    );
    ...
    const reloadedExpCookies = reloadedCookies.filter(
      c => c.name.includes('ab_') || c.name.includes('experiment')
    );
...
test.describe('E-commerce Tracking', () => {
  ...
  test('should track pricing plan selection', async ({ page }) => {
    ...
    const planButton = page
      .locator('button:has-text("Choose Plan"), button:has-text("Get Started")')
      .first();
