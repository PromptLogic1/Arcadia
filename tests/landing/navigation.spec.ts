...
test.describe('Navigation - Header & Footer', () => {
  ...
  test('should have functioning mobile menu', async ({ page }) => {
    ...
    if (await menuTrigger.count() > 0) {
      ...
      if (await mobileMenu.count() > 0) {
        ...
        // Close menu
        const closeButton = page.locator('button[aria-label*="close"], button:has([data-lucide="x"]), [data-testid="mobile-menu-close"]');
  ...
  test('should highlight active navigation items', async ({ page }) => {
    ...
    if (await aboutLink.count() > 0) {
      ...
      const activeAboutLink = page.locator('a[href="/about"], a:has-text("About")').first();
  ...
  test('should display user menu when authenticated', async ({ page }) => {
    ...
    const userMenu = page.locator('[data-testid="user-menu"], .user-menu, button:has-text("Test"), [aria-label*="user"]');
  ...
  test('should display auth buttons when not authenticated', async ({ page }) => {
    ...
    const authButtons = page.locator('a[href*="login"], a[href*="signup"], button:has-text("Login"), button:has-text("Sign In"), button:has-text("Sign Up")');
  ...
  test('should have theme toggle functionality', async ({ page }) => {
    ...
    const themeToggle = page.locator('[data-testid="theme-toggle"], button[aria-label*="theme"], button:has([data-lucide="sun"]), button:has([data-lucide="moon"])');
  ...
  test('should handle search functionality if present', async ({ page }) => {
    ...
    const searchTrigger = page.locator('input[type="search"], [data-testid="search"], button[aria-label*="search"], button:has([data-lucide="search"])');
...
test.describe('Navigation - Footer', () => {
  ...
  test('should display footer with required links', async ({ page }) => {
    ...
    for (const linkText of footerLinks) {
      const link = page.locator(`footer a:has-text("${linkText}"), [role="contentinfo"] a:has-text("${linkText}")`);
  ...
  test('should handle external links securely', async ({ page }) => {
    ...
    const externalLinks = page.locator('footer a[href^="http"], [role="contentinfo"] a[href^="http"]');
  ...
  test('should display social media links if present', async ({ page }) => {
    ...
    for (const pattern of socialMediaPatterns) {
      const socialLink = page.locator(`footer a[href*="${pattern}"], [role="contentinfo"] a[href*="${pattern}"]`);
  ...
  test('should contain copyright information', async ({ page }) => {
    ...
    const copyrightText = page.locator('footer text=/©|Copyright|All rights reserved/, [role="contentinfo"] text=/©|Copyright|All rights reserved/');
...
test.describe('Navigation - User Journey', () => {
  test('should support complete landing to play journey', async ({ page }) => {
    ...
    const startPlayingButton = page.locator('button:has-text("Start Playing"), a:has-text("Start Playing"), button:has-text("Play Now")').first();
  ...
  test('should support landing to community journey', async ({ page }) => {
    ...
    const communityButton = page.locator('button:has-text("Join Community"), a:has-text("Community"), a[href*="community"]').first();
