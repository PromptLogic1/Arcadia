...
test.describe('Enhanced Performance Testing', () => {
  ...
  test('should meet all performance budgets (2024 Core Web Vitals) @critical @performance', async ({
    page,
  }) => {
    await page.goto('/');
    ...
    const gradeColor =
      metrics.performanceGrade === 'Good'
        ...
        : metrics.performanceGrade === 'Needs Improvement'
          ...
          : '🔴';
    console.log(
      `${gradeColor} Overall Performance Grade: ${metrics.performanceGrade}`
    );
  ...
  test('should optimize resource loading', async ({ page }) => {
    ...
    if (resourceBreakdown.images.count > 0) {
      const avgImageSize =
        resourceBreakdown.images.totalSize / resourceBreakdown.images.count;
  ...
  test('should handle concurrent user load efficiently', async ({
    browser,
  }) => {
  ...
  test('should optimize critical rendering path', async ({ page }) => {
    ...
    page.on('response', response => {
      ...
      const timing = (
        response as unknown as { timing?: () => { responseEnd?: number } }
      ).timing?.();

      if (
        timing &&
        typeof timing.responseEnd === 'number' &&
        timing.responseEnd < 1000
      ) {
        // Resources loaded within 1s
        if (
          url.includes('.css') ||
          url.includes('.js') ||
          url.includes('font')
  ...
  test('should maintain performance with real user interactions and measure INP @performance @inp', async ({
    page,
  }) => {
    await page.goto('/');
          ...
          const observer = new PerformanceObserver(list => {
            ...
            entries.forEach(entry => {
              ...
              if (eventEntry.interactionId) {
                const inp =
                  (eventEntry.processingStart || 0) - eventEntry.startTime;
    ...
    if (inpData.length > 0) {
      ...
      const avgINP =
        inpValues.reduce((sum: number, val: number) => sum + val, 0) /
        inpValues.length;
      ...
      inpData.forEach(
        (measurement: { target: string; inp: number }, index: number) => {
          console.log(
            `  ${index + 1}. ${measurement.target}: ${measurement.inp.toFixed(2)}ms`
          );
        }
      );
    ...
    console.log(
      `  Memory before: ${(initialMetrics.jsHeapUsed / 1024 / 1024).toFixed(2)}MB`
    );
    console.log(
      `  Memory after: ${(afterMetrics.jsHeapUsed / 1024 / 1024).toFixed(2)}MB`
    );
    console.log(
      `  Memory increase: ${((afterMetrics.jsHeapUsed - initialMetrics.jsHeapUsed) / 1024 / 1024).toFixed(2)}MB`
    );
