...
test.describe('Responsive Design - Mobile & Tablet', () => {
  ...
  for (const viewport of viewports) {
    test(`should render properly on ${viewport.name} (${viewport.width}x${viewport.height})`, async ({
      ...
      const ctaButtons = page.locator(
        'button:has-text("Start Playing"), button:has-text("Play"), a:has-text("Start Playing")'
      );
  ...
  test('should stack elements vertically on mobile', async ({ page }) => {
    ...
    const gridContainer = page
      .locator('.grid, .flex, [class*="grid-cols"]')
      .first();
    ...
    if ((await gridContainer.count()) > 0) {
      ...
      if (itemCount >= 2) {
        ...
        if (firstItem && secondItem) {
          ...
          expect(secondItem.y).toBeGreaterThan(
            firstItem.y + firstItem.height - 10
          ); // Should be below
        }
      }
    }
  ...
  test('should handle tablet landscape orientation', async ({ page }) => {
    ...
    const container = page
      .locator('.container, main, [class*="max-w"]')
      .first();
  ...
  test('should adapt carousel/slider for touch devices', async ({ page }) => {
    ...
    const carousel = page
      .locator('.carousel, .slider, [class*="snap"], .swiper')
      .first();
      ...
      if ((await button1.isVisible()) && (await button2.isVisible())) {
        ...
        if (box1 && box2) {
          ...
          if (verticalDistance < 50) {
            // If they're roughly in the same row
...
test.describe('Responsive Design - Cross-browser', () => {
  test('should maintain layout consistency across browsers', async ({
    page,
    ...
    const layoutContainers = page.locator(
      '.grid, .flex, [style*="grid"], [style*="flex"]'
    );
...
test.describe('Responsive Design - Accessibility', () => {
  test('should maintain focus visibility at all viewport sizes', async ({
    page,
  }) => {
    const viewports = [
      { width: 375, height: 667 },
  ...
  test('should support screen reader navigation at all sizes', async ({
    page,
  }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    await page.goto('/');
