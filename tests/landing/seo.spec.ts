...
import type { StructuredDataSchema, SEOValidation } from './types';
    ...
    schemas: [
      {
        ...
        validation: {
          ...
          name: (value: unknown) =>
            typeof value === 'string' && (value as string).includes('Arcadia'),
          url: (value: unknown) =>
  ...
  Organization: {
    ...
    validation: {
      ...
      logo: (value: unknown) =>
        !value ||
        (typeof value === 'string' &&
          /\.(jpg|jpeg|png|svg|webp)(\?|$)/i.test(value as string)),
...
test.describe('Enhanced SEO & Structured Data Validation', () => {
  ...
  test('should have comprehensive meta tags validation @seo @critical', async ({
    page,
  }) => {
    // Title validation
    ...
    expect(metaDescription!.length).toBeGreaterThanOrEqual(
      SEO_CONFIG.description.minLength
    );
    expect(metaDescription!.length).toBeLessThanOrEqual(
      SEO_CONFIG.description.maxLength
    );
  ...
  test('should have valid Open Graph tags @seo @social', async ({ page }) => {
    for (const ogTag of SEO_CONFIG.openGraph) {
      ...
      if (ogTag.required) {
        expect(
          content,
          `Missing required Open Graph tag: ${ogTag.property}`
        ).toBeTruthy();
      }
  ...
  test('should have valid Twitter Card tags @seo @social', async ({ page }) => {
    for (const twitterTag of SEO_CONFIG.twitter) {
      ...
      if (twitterTag.required) {
        expect(
          content,
          `Missing required Twitter tag: ${twitterTag.property}`
        ).toBeTruthy();
      }
      ...
      if (content) {
        ...
        if (
          twitterTag.property === 'twitter:site' ||
          twitterTag.property === 'twitter:creator'
        ) {
  ...
  test('should have comprehensive JSON-LD structured data @seo @structured-data', async ({
    page,
  }) => {
    ...
    for (const script of jsonLDScripts) {
      ...
      for (const data of dataArray) {
        ...
        if (STRUCTURED_DATA_SCHEMAS[schemaType]) {
          await validateStructuredDataSchema(
            data,
            STRUCTURED_DATA_SCHEMAS[schemaType]
          );
        }
      }
    }
    ...
    for (const requiredType of SEO_CONFIG.structuredData.types) {
      expect(
        foundSchemas.has(requiredType),
        `Missing required structured data type: ${requiredType}`
      ).toBe(true);
    }
  ...
  test('should have valid Organization schema @seo @structured-data', async ({
    page,
  }) => {
    const organizationData = (await getStructuredDataByType(
      page,
      'Organization'
    )) as Record<string, unknown> | null;
    expect(
      organizationData,
      'Organization schema should be present'
    ).toBeTruthy();
    ...
    if (organizationData) {
      ...
      if (organizationData.logo) {
        expect(organizationData.logo).toMatch(
      ...
      if (organizationData.contactPoint) {
        const contactPoint = organizationData.contactPoint as Record<
          string,
          unknown
        >;
  ...
  test('should have valid WebSite schema with search action @seo @structured-data', async ({
    page,
  }) => {
    const websiteData = (await getStructuredDataByType(
      page,
      'WebSite'
    )) as Record<string, unknown> | null;
    ...
    if (websiteData) {
      ...
      if (websiteData.potentialAction) {
        const searchAction = Array.isArray(websiteData.potentialAction)
          ? (
              websiteData.potentialAction as Array<Record<string, unknown>>
            ).find(action => action['@type'] === 'SearchAction')
  ...
  test('should have proper mobile SEO optimization @seo @mobile', async ({
    page,
  }) => {
    // Set mobile viewport
  ...
  test('should have proper international SEO setup @seo @i18n', async ({
    page,
  }) => {
    // Language attributes
    const htmlLang = await page.locator('html').getAttribute('lang');
    ...
    if (hreflangTags.length > 0) {
      ...
      const _hasXDefault = hreflangTags.some(
        async tag => (await tag.getAttribute('hreflang')) === 'x-default'
      );
      // Note: This is optional for single-language sites
    }

    // Content language meta tag
    const contentLanguage = await page
      .locator('meta[http-equiv="content-language"]')
      .getAttribute('content');
    if (contentLanguage) {
  ...
  test('should have valid schema.org breadcrumb navigation @seo @navigation', async ({
    page,
  }) => {
    ...
    const breadcrumbData = (await getStructuredDataByType(
      page,
      'BreadcrumbList'
    )) as Record<string, unknown> | null;
    ...
    if (breadcrumbData && breadcrumbData.itemListElement) {
      ...
      const items = breadcrumbData.itemListElement as Array<
  ...
  test('should have optimized page loading for search engines @seo @performance', async ({
    page,
    context,
  }) => {
...
// Helper functions
async function validateStructuredDataSchema(
  data: unknown,
  ...
  if (schema.validation) {
    for (const [field, validator] of Object.entries(schema.validation)) {
      if (dataObj[field] !== undefined) {
        ...
        expect(
          isValid,
          `Validation failed for field: ${field} with value: ${dataObj[field]}`
        ).toBe(true);
      }
    }
  }
    ...
    if (content) {
      try {
        ...
        const found = dataArray.find(
          (item: unknown) => (item as Record<string, unknown>)['@type'] === type
        );
