...
test.describe('Basic UI Smoke Tests', () => {
  ...
  test('theme toggle works', async ({ page }) => {
    ...
    const mobileToggle = page
      .locator('button[aria-label*="Switch to"][aria-label*="theme"]')
      .first();
  ...
  test('forms have proper validation', async ({ page }) => {
    ...
    if (await submitButton.isVisible()) {
      ...
      const errorMessages = page.locator(
        '[role="alert"], .error-message, [aria-invalid="true"], .text-red-500, .text-destructive'
      );
  ...
  test('images have alt text', async ({ page }) => {
    ...
    const percentageWithAlt =
      ((imagesToCheck - imagesWithoutAlt) / imagesToCheck) * 100;
