  ...
  test.describe('Landing Page', () => {
    test('hero section displays correctly', async ({ page }) => {
      ...
      const ctaButtons = page
        .locator('button, a')
        .filter({ hasText: /start playing|join community/i });
    ...
    test('featured games carousel works', async ({ page }) => {
      ...
      const carousel = page
        .locator(
          '[data-testid*="carousel"], [class*="carousel"], [class*="slider"], [role="region"][aria-label*="carousel"]'
        )
        .first();
    ...
    test('upcoming events section loads', async ({ page }) => {
      ...
      const eventsSection = page
        ...
        .filter({
          has: page
            .locator('h2, h3')
            .filter({ hasText: /event|upcoming|tournament/i }),
        })
        .first();

      if (await eventsSection.isVisible()) {
        // Check for event cards/items
        const eventItems = eventsSection.locator(
  ...
  test.describe('Authentication Pages', () => {
    test('login page loads and has required elements', async ({ page }) => {
      ...
      const signUpLink = page
        .locator('a')
        .filter({
          hasText: /sign up|create account|register|don't have an account/i,
        })
        .first();
    ...
    test('signup page loads and has required elements', async ({ page }) => {
      ...
      const heading = page
        .locator('h2')
        .filter({
          hasText: /join arcadia|create your account|sign up|register/i,
        })
        .first();
      await expect(heading).toBeVisible({ timeout: 10000 });
  ...
  test.describe('Play Area', () => {
    test('play area hub loads', async ({ page }) => {
      ...
      const gameCards = page.locator(
        '[data-testid*="game"], [class*="game-card"], article[class*="card"]'
      );
      ...
      if (gameCount > 0) {
        ...
        const firstGameTitle = gameCards
          .first()
      ...
      const filters = page.locator(
        '[role="tablist"], [data-testid*="filter"], [class*="category"]'
      );
  ...
  test.describe('Community Section', () => {
    test('community page loads with discussions', async ({ page }) => {
      ...
      const createButton = page
        .locator('button, a')
        .filter({ hasText: /create|new discussion|post/i })
        .first();

      // Check for discussion items
      const discussions = page.locator(
        '[data-testid*="discussion"], [class*="discussion"], article[class*="post"]'
      );
  ...
  test.describe('User Settings', () => {
    test('settings page is accessible', async ({ page }) => {
      ...
      if (currentUrl.includes('/auth/login')) {
        ...
        await expect(
          page.locator('h2').filter({ hasText: /welcome back/i })
        ).toBeVisible();
      } else {
        ...
        await expect(
          page
            .locator('h1, h2')
            .filter({ hasText: /settings|preferences|profile/i })
            .first()
        ).toBeVisible({ timeout: 10000 });

        // Should have settings sections
        const settingsSections = page.locator(
          '[role="tablist"], [data-testid*="settings"], section[class*="settings"]'
        );
  ...
  test.describe('Error Handling', () => {
    test('handles API errors gracefully', async ({ page }) => {
      ...
      const errorNotifications = page.locator(
        '[role="alert"], [data-testid*="error"], .error-message'
      );
    ...
    test('handles network offline gracefully', async ({ page, context }) => {
      ...
      const offlineMessage = page.locator(
        'text=/offline|no internet|connection/i'
      );
      const isOfflineMessageVisible = await offlineMessage
        .isVisible()
        .catch(() => false);

      // Page should handle offline state somehow
      expect(
        isOfflineMessageVisible || page.url().includes('/about')
      ).toBeTruthy();
    });
  });
});
